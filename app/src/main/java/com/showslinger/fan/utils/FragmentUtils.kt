...
object FragmentUtils {
    ...
    fun Fragment.isDestroyed(): Boolean {
        return activity == null || activity?.isFinishing == true || isDetached || view == null
    }
    ...
    private fun <T : Any> run(option: ReplaceOption<T>, isAddFragment: Boolean) {
        ...
        val tagName = option.frag::class.java.name
        ...
        if (isAddFragment) {
            ...
            transaction.add(option.mcontainerId, option.frag, tagName)
        } else {
            transaction.replace(option.mcontainerId, option.frag, tagName)
        }
